...
def RowNumberAndColumnNumberToPosition(row, column) -> tuple:
	x = (column-row)*65 + 655
	y = (row+column)*32 + 258
...
class Tile(object):
	...
	def __init__(self, width, height, rowNumber, columnNumber):
		...
		self.position = self.x, self.y = RowNumberAndColumnNumberToPosition(self.rowNumber, self.columnNumber)
		...
		self.selected = False
		self.unselectedY = self.y
		self.selectedY = self.y - 10
		
		self.unselectedSurface = self.image
		...
		self.cornersOfSelectedPolygon[0] = self.cornersOfSelectedPolygon[0][0], self.cornersOfSelectedPolygon[0][1] - self.y + self.selectedY
		self.cornersOfSelectedPolygon[1] = self.cornersOfSelectedPolygon[1][0], self.cornersOfSelectedPolygon[1][1] - self.y + self.selectedY
		self.cornersOfSelectedPolygon[2] = self.cornersOfSelectedPolygon[2][0], self.cornersOfSelectedPolygon[2][1] - self.y + self.selectedY
		self.cornersOfSelectedPolygon[3] = self.cornersOfSelectedPolygon[3][0], self.cornersOfSelectedPolygon[3][1] - self.y + self.selectedY
	...
	def Draw(self, Window: pygame.Surface, mousePosition, isThereAnySelectedTile, infoMode):

		isMouseOver = self.isMouseOver(mousePosition)

		if isMouseOver:
			if not isThereAnySelectedTile:
				
				self.surface = self.selectedSurface
				
				if infoMode:
					self.selected = True
					self.rect.y = self.selectedY
		else:
			self.selected = False
			self.rect.y = self.unselectedY
			self.surface = self.unselectedSurface
